...
describe("Touch shell through the mounted PocketJS guest", () => {
  test("content, home, overview, quick switch and back share live mounted windows", async () => {
    ...
    const nodes: number[] = [];
    function visit(n: any) { if (n.n?.startsWith("TouchWindow")) nodes[Number(n.n.slice(-1))] = n.i; n.k?.forEach(visit); }
    visit(initialTree);
    expect(nodes).toHaveLength(6);
    for (const name of ["Weather", "Notes", "Photos"]) expect(treeHasText(initialTree, name)).toBe(true);
    ...
    for (const [index, x] of [[3, 56], [4, 160], [5, 264]]) {
      glide(160, 466, 160, 425);
      tap(x, 276);
      expect(prop(index, PROP.scaleX)).toBe(1);
      expect(prop(index, PROP.opacity)).toBe(1);
      for (const other of [0, 1, 2, 3, 4, 5].filter(i => i !== index)) expect(prop(other, PROP.opacity)).toBe(0);
      const before = Bun.hash(world.render());
      glide(160, 380, 160, 200);
      expect(Bun.hash(world.render())).not.toBe(before);
    }
